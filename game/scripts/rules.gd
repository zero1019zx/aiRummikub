...
## 校验一组连续摆放的牌是否为合法牌组
## 返回 { "valid": bool, "kind": "run"/"group"/"pair"/"", "values": Array[int] 每张牌的计分值(Joker取代表值) }
## 对子(同数字2张, 任意颜色)为合法的"半成品": 上桌不计chips, 但可作重组燃料/补成刻子
static func check_set(tiles: Array) -> Dictionary:
	var bad := {"valid": false, "kind": "", "values": []}
	if tiles.size() < 2:
		return bad
	if tiles.size() == 2:
		return _check_pair(tiles)
...
## 对子: 同数字2张(颜色不限, 含两张完全相同), Joker可顶1张
static func _check_pair(tiles: Array) -> Dictionary:
	var num := -1
	for t in tiles:
		if t.joker:
			continue
	...
	if num == -1:
		return {"valid": false}
	return {"valid": true, "kind": "pair", "values": [num, num]}
