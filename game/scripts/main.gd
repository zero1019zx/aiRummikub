...
var arena_db: ArenaDB = null         # 资产加载器(懒加载, data/*.json)
var arena_boss: Dictionary = {}      # 当前层选中的boss定义(空=未启用/回退到原对战)
var arena_floors: Array = []         # 阶段门血量边界
var arena_phase := 0                 # 当前阶段索引
...
func _start_floor() -> void:
	...
	arena_boss = {}                      # 每层重选boss(非arena则保持空)
	score = 0
	turn = 1
...
# ---------- 场面争夺: Boss运行时 (数据驱动 + 全程容错; 缺数据自动回退原对战) ----------
func _ensure_arena_db() -> void:
	if arena_db == null:
		arena_db = ArenaDB.new()
		arena_db.load_all()

## 是否真正以"数据驱动boss"模式运行(arena开启且成功选到boss)
func _arena_active() -> bool:
	return arena_mode and not arena_boss.is_empty()

## 选本层boss; 成功返回true。失败(无db/无匹配)返回false → 外层回退原对战
func _arena_setup_boss() -> bool:
	arena_boss = {}
	arena_floors = []
	arena_phase = 0
	_ensure_arena_db()
	...
	if cands.is_empty():
		return false
	arena_boss = cands[randi() % cands.size()]
	arena_floors = arena_db.boss_phase_floors(arena_boss)
	return true

func _arena_boss_maxhp() -> int:
	return arena_db.boss_total_hp(arena_boss) if _arena_active() else _battle_max()

func _player_max() -> int:
	if _arena_active():
		return int(arena_db.player_cfg().get("max_hp", 80))
	return PLAYER_MAX_HP

func _arena_regen() -> int:
	return int(arena_db.player_cfg().get("regen_per_turn", 12)) if _arena_active() else 0

func _arena_draw_per_turn() -> int:
	return int(arena_db.player_cfg().get("draw_per_turn", 2)) if _arena_active() else 1

func _arena_cur_phase() -> Dictionary:
	if not _arena_active():
		return {}
	var phs: Array = arena_boss.get("phases", [])
	return phs[arena_phase] if arena_phase < phs.size() else {}

func _arena_cur_threat() -> int:
	return int(_arena_cur_phase().get("threat", 12))

func _arena_cur_floor() -> int:
	return int(arena_floors[arena_phase]) if arena_phase < arena_floors.size() else 0

func _arena_mech(type: String) -> Dictionary:
	for m in _arena_cur_phase().get("mechanics", []):
		if m.get("type", "") == type:
			return m
	return {}

## 步骤3 机制: 对"打给Boss的伤害"整形 — 韧性/护甲/霸体单击上限(均为局部, 非全局软上限)
func _arena_shape_damage(d: int) -> int:
	for m in _arena_cur_phase().get("mechanics", []):
		match m.get("type", ""):
			"armor":
				if d < int(m.get("pierce_at", 1 << 30)):
					d = max(0, d - int(m.get("value", 0)))
			"resist":
				var soft := int(m.get("soft", 60))
				if d > soft:
					d = soft + int((d - soft) * float(m.get("k", 0.5)))
			"cap_hit_pct":
				d = min(d, int(round(_arena_boss_maxhp() * float(m.get("pct", 0.3)))))
	return d

## 步骤2 阶段门 + 步骤3 溢出反弹: 把本回合伤害结算到Boss。返回[实际生效, 反弹给玩家]; 并推进阶段。
func _arena_deal_to_boss(raw: int) -> Array:
	var shaped := _arena_shape_damage(raw)
	var floorv := _arena_cur_floor()
	var eff: int = max(0, min(shaped, enemy_hp - floorv))   # 阶段门: 不击穿本阶段血量下限
	var overflow: int = shaped - eff
	var phs: Array = arena_boss.get("phases", [])
	var reflect := 0
	var rm := _arena_mech("overflow_reflect")
	# 仅"最后阶段的过量斩杀"才反弹(收残血失手挨打); 阶段内溢出只是浪费, 不反伤
	if overflow > 0 and not rm.is_empty() and arena_phase >= phs.size() - 1:
		reflect = int(overflow * float(rm.get("pct", 0.5)))
	enemy_hp -= eff
...
func _on_end_turn() -> void:
	...
	if mode != "score":
		if _arena_active():
			# 场面争夺: 伤害先过机制整形 + 阶段门, 再结算到boss; 处理溢出反弹
			var res_dmg := _arena_deal_to_boss(turn_gain)
			turn_gain = int(res_dmg[0])
			var reflect := int(res_dmg[1])
			if turn_gain > 0:
				var heal := relic_mgr.lifesteal(turn_gain)
				if heal > 0:
					player_hp = min(_player_max(), player_hp + heal)
				_shake(ebox)
			if reflect > 0:
				player_hp -= reflect
				_toast("溢出反弹 %d!" % reflect)
				_shake(pbar.root)
			_update_hud()
			if player_hp <= 0:
				player_hp = 0
			...
			if enemy_hp <= 0:
				...
				_show_floor_result(true)
				return
		var draws_b := relic_mgr.draw_count(_arena_draw_per_turn())
		for _i in draws_b:
			_draw_to_rack()
		var alive := true
		if mode == "tutorial":
...
func _update_hud() -> void:
	...
	if mode != "score":
		...
		_update_bar(pbar, "我方", player_hp, _player_max())
		if _arena_active():
			_update_bar(ebar, String(arena_boss.get("name", "Boss")), enemy_hp, _arena_boss_maxhp())
			lbl_turn_b.text = "阶段 %d/%d" % [arena_phase + 1, int(arena_boss.get("phases", []).size())]   # boss阶段(头像下方)
		else:
